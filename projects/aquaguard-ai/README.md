...
设置 `AQUAGUARD_OPERATOR_CREDENTIAL_DATABASE_PATH` 可启用动态 SQLite 凭据仓库；空库首次启动时由 `AQUAGUARD_OPERATOR_CREDENTIALS` 事务化引导，之后数据库成为权威来源。管理员可通过 `/api/v1/operator-credentials` 新增和脱敏查看凭据，并吊销旧凭据。每次受保护请求认证前会读取单调版本号；共享同一 SQLite 文件的其他服务进程会在发现版本变化后重新加载凭据。系统拒绝重复指纹、已过期的新凭据、自我吊销和吊销最后一个有效管理员。成功及被拒绝的管理动作会写入安全审计。查询接口不会返回 `token_sha256`。
